...
//Code Here
let postTaxPrices = prices.map(val => val * 1.07); // = prices.map(/* Provide Your Callback Here );
...
//Code Here
let totalPopulation = populations.reduce((acc, val, i, arr) => acc + val); //  = populations.reduce(/* Provide Your Callback Here */)
...
let ordersTotal = orders.map(val => val.price * (1 + val.tax)); //Code Here
...
let bobsTotal = purchases
  .filter(val => val.owner === "Bob")
  .reduce((acc, val) => acc + val.price, 0); //Code Here
